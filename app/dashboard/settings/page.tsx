...
// ИЗМЕНЕНО: раньше промпт требовал отвечать строго по базе знаний и звать
// менеджера при нехватке информации — это конфликтует с новым поведением
// бота (main.py/qwen_client.py), где он отвечает на любые вопросы своими
// знаниями, а эскалация — крайний случай.
const DEFAULT_PROMPT =
  "Ты — вежливый и компетентный ассистент компании. Приоритетно используй базу знаний компании, но свободно отвечай и на общие/бытовые вопросы своими знаниями, как обычный живой оператор поддержки. Зови менеджера только в действительно исключительных случаях: действия с личным аккаунтом/оплатой/возвратом денег, жалобы, либо когда не можешь дать достоверный ответ.";
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
        ...
        <div>
          ...
          />
          <p className="text-xs text-neutral-500 mt-1.5">
            Детальные директивы модели. Бот теперь отвечает на любые вопросы (включая
            бытовые), а не только строго по базе знаний — используйте этот промпт, чтобы
            задать тон и границы, а не заставлять бота молчать вне базы.
          </p>
        </div>
        ...
        <div className="pt-2 border-t border-neutral-100">
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-neutral-900 flex items-center gap-2">
              ...
              Порог уверенности RAG (влияет только на релевантность базы знаний)
            </label>
          ...
          {/* ИЗМЕНЕНО: раньше этот порог решал, отвечает бот сам или сразу
              эскалирует диалог. Теперь бот всегда пытается ответить сам, а
              эскалацию решает модель по ситуации (см. main.py). Значение
              сохраняется для будущего использования, но на решение об
              эскалации сейчас не влияет — пояснение показано явно, чтобы не
              вводить владельца бота в заблуждение. */}
          <p className="text-xs text-neutral-600 mt-2 bg-neutral-50 p-3 rounded-xl border border-neutral-200">
            Этот параметр больше не решает, эскалировать диалог на менеджера или нет —
            бот теперь всегда старается ответить сам (в том числе на бытовые вопросы вне
            базы знаний). Эскалация происходит только в исключительных случаях: явная
            просьба позвать человека, вопросы про оплату/аккаунт/возврат, жалобы, либо
            когда бот не может дать достоверный ответ.
          </p>
        </div>
